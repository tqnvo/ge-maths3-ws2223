...
index $n=0$, and the second part with $n\geq 1$, as follows
\begin{align}
	x(t_j+lh) 
...
\begin{align}
	x'(t_j+lh) 
	 ...
	 & = \sum_{n=1}^{r+1} \frac{x^{(n)}(t_j)\,(lh)^{n-1}}{(n-1)!} + \mathcal{O}((lh)^{r+1}) \notag     \\
	 & = \sum_{n=1}^{r+1} \frac{n\,x^{(n)}(t_j)\,(lh)^{n-1}}{n!} + \mathcal{O}((lh)^{r+1}) \notag      \\
	 & = \sum_{n=1}^{r+1} \frac{n\,x^{(n)}(t_j)\,l^{n-1}h^{n}}{n!\,h} + \mathcal{O}((lh)^{r+1}) \notag \\
\end{align}
\newpage
\begin{observationboxed}
