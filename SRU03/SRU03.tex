...
Mathe III $\cdot$ SRU01 $\cdot$ Response to questions $\cdot$ WS22/23 
\hspace{280pt} 
...
\begin{observationboxed}
	There are 6 symbols arising in \eqref{eq:multistepform} to be aware of
	\begin{enumerate}
		\item $m$ is the total number of steps used in the multistep method.
		\item $\alpha \in \mathbb{R}$ is the coefficient, arising in the linear combination of $x(t_j+lh)$
		\item $\beta \in \mathbb{R}$ is the coefficient, arising in the linear combination of $f(t_j+lh,x(t_j+lh))$.
		\item $p$ is the consistency order.
		\item $l$ is the dummy running index, from $0$ to $m$.
		\item $q$ is the dummy running index, from $1$ to $p$.
	\end{enumerate}
...
\begin{observationboxed}
	...
	there is actually a \textbf{linear combination} of the term $x(t_j+lh)$ with coefficients $\alpha_l$
	on the LHS of \eqref{eq:multistepform}, and 
	a \textbf{linear combination} of the term $f(t_j+lh,x(t_j+lh))$
	...
	\begin{align}
		...
		\notag    \\
		 & \quad 
		+ \beta_{m}\,\textcolor{Bittersweet}{f(t_j+mh,x(t_j+mh))}.
	\end{align}
...
%------------------------------------------------------------------------------
\newpage
\section{Step-by-step with A4-SRU03}
\begin{exampleboxed}
	Examine the consistency order of the following explicit numerical scheme 
	\begin{align}
		y^{j+1} = y^{j}+\frac{h}{2}\Big(f(t^j,y^j)+f(t^{j+1},y^j+hf(t^j,y^j))\Big),
	\end{align}
	which is used to approximate the ordinary differential equation (ODE):
	\begin{align}
		y^{\prime}(t)=f(t,y(t)).
	\end{align}
\end{exampleboxed}
...
Approach:
The exact solution at time $t+h$ is obtained by Taylor series expansion as
\begin{align*}
...
\begin{align*}
	y^\prime         
	 & = f\big(t,y\big)                               \\
	y^{\prime\prime}
	 & = \frac{\mathrm{d}}{\mathrm{d}t}f\big(t,y\big)
	= f_t\big(t,y(t)\big) + f_y\big(t,y(t)\big) y^\prime(t)
	= f_t\big(t,y(t)\big) + f_y\big(t,y(t)\big) f\big(t,y(t)\big).
\end{align*}
Thus
\begin{align*}
	y(t+h) = 
	y^j 
	+ h\, f(t,y^j) 
	+ \frac{h^2}{2}\Big(f_t(t,y^j) + f_y(t,y^j)\,f(t,y^j)\Big)+\mathcal{O}(h^3)
\end{align*}
The approximation can be written as
\begin{align*}
	y^{j+1} = y^j + h\,\varPhi(h), \quad\textrm{with}\quad\varPhi(h) = \frac{1}{2}\Big[f(t,y^j)+f(t+h,y^j+hf(t,y^j))\Big].
\end{align*}
We now expand $\varPhi(h)$ around $0$, i.e., $\varPhi(h) = \varPhi(0) + h\,\varPhi^\prime(0) + \mathcal{O}(h^2)$. Here
\begin{align*}
	\varPhi^\prime(h) = \frac{1}{2}f_t(t + h, y^j+hf(t,y^j)) + \frac{1}{2}f_y(t + h, y^j + h f(t,y^j)) f(t,y^j).
\end{align*}
Thus
\begin{align*}
	\varPhi(0) = f(t,y^j),\quad \varPhi^\prime(0) = \frac{1}{2}f_t(t, y^j) + \frac{1}{2}f_y(t, y^j) f(t,y^j)
\end{align*}
and
\begin{align*}
	\varPhi(h) = f(t,y^j) +\frac{h}{2}\Big(f_t(t, y^j) + f_y(t,y^j) \,f(t,y^j)\Big) + \mathcal{O}(h^2).
\end{align*}
Therefore
\begin{align*}
	y^{j+1} = y^j + h\, f(t,y^j) + \frac{h^2}{2}\Big(f_t(t,y^j) + f_y(t,y^j)\,f(t,y^j)\Big)+\mathcal{O}(h^3).
\end{align*}
The error $\big|y(t+h) - y^{j+1}\big| = \mathcal{O}(h^3)$ and thus the method has consistency order $ 2 $.
% \begin{align}
