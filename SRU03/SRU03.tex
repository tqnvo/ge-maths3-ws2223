...
\begin{align}
	...
	\\
	x'(t_j+lh) & = \sum_{n=0}^{r} \frac{x^{(n+1)}(t_j)\,(lh)^{n}}{n!} + \mathcal{O}((lh)^{r+1}),
...
\eqref{eq:taylorsubder}, meaning that
\begin{align}
	a & \rightarrow t_j, \\
	h & \rightarrow lh. 
\end{align}
Furthermore, expression \eqref{eq:taylorsubderno} can be split into two main parts, where the first part going with 
index $n=0$ and the second part going with $n\geq 1$, as follows
\begin{align}
	x(t_j+lh) 
	 & = \sum_{n=0}^{q} \frac{x^{(n)}(t_j)\,(lh)^{n}}{n!} + \mathcal{O}((lh)^{q+1}) \notag                                     \\
	 & = \frac{x^{(0)}(t_j)\,(lh)^{0}}{0!} + \sum_{n=1}^{q} \frac{x^{(n)}(t_j)\,(lh)^{n}}{n!} + \mathcal{O}((lh)^{q+1}) \notag \\
	 & = x(t_j) + \sum_{n=1}^{q} \frac{x^{(n)}(t_j)\,(lh)^{n}}{n!} + \mathcal{O}((lh)^{q+1}). 
\end{align}
Besides, shifting index starting from $i=0$ to $i=1$ for expression \eqref{eq:taylorsubder} yields
\begin{align}
	x'(t_j+lh) 
	 & = \sum_{n=0}^{r} \frac{x^{(n+1)}(t_j)\,(lh)^{n}}{n!} + \mathcal{O}((lh)^{r+1}) \notag \\
