...
Mathe III $\cdot$ GUE08 $\cdot$ WS22/23 
\hspace{220pt} 
...
\begin{align}
	...
	\quad
	\begin{cases}
		r_2 = \sqrt{a^2+b^2} = \sqrt{B_{11}^2 + B_{31}^2} = \sqrt{26}, \\
		c_2 =  a_2/r_2 = 5/\sqrt{26},                                      \\
		s_2 = -b_2/r_2 = -1/\sqrt{26},                                     \\
	\end{cases}
\end{align}
...
\begin{align}
	G_{31}A = R 
	\Leftrightarrow
	G_{31}^{-1}G_{31}A = G_{31}^{-1}R
	\Leftrightarrow
	A = G_{31}^{-1}R
	\Leftrightarrow
	A = G_{31}^{\top}R.
\end{align}
Note in passing that $G_{31}^{-1} = G_{31}^{\top}$ in the previous step is due to the fact 
that the matrix $G_{31}$ itself is orthogonal. 
By assigning $Q:=G_{31}^{\top}$ we arrive at QR-decomposition
\begin{align*}
	\therefore\quad\boxed{
		A = G_{31}^{\top}R = QR.
	}
...
% Die Ungleichung gilt für alle Paare $p,q\in[1,\infty]$ mit $\tfrac{1}{p} + \tfrac{1}{q} = 1$.
% Hierbei setzt man~$\tfrac{1}{\infty} := 0$.

% Wir setzen $u(x) := (x+2)^{-\tfrac{3}{5}}$ und $v(x) := e^{-\tfrac{2}{3}x}$. Dann gilt
% $u\in L^6$:
% \begin{equation*}
...
% $\tfrac{1}{6}+\tfrac{1}{q} = 1\iff q = \tfrac{6}{5}$. Dann gilt $v\in L^q(\Omega)$:
% \end{enumerate}
% \begin{equation*}
% 	\Vert v\Vert_{L^{\frac{6}{5}}(\Omega)}^{\tfrac{6}{5}}
% 	= \int_{0}^{\infty} e^{-\frac{2}{3}\cdot\frac{6}{5}\cdot x}\,{\mathrm d}x
% 	= -\left.\frac{5}{4}e^{-\frac{4}{5}x}\right|_{x=0}^{x=\infty} = \frac{5}{4} < \infty.    
% \end{equation*}

% Insgesamt erhalten wir:
% \begin{equation*}
% 	\int_{\Omega} \frac{1}{\sqrt[5]{(x + 2)^{3}}} e^{-\frac{2}{3} x}\,{\mathrm d}x
% 	=   \Vert uv\Vert_{L^1(\Omega)}
% 	\leq \Vert u\Vert_{L^p(\Omega)}\Vert v\Vert_{L^q(\Omega)} < \infty.
% \end{equation*}
%------------------------------------------------------------------------------
